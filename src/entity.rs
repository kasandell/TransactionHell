...
impl User {
    ...
    pub async fn insert_accept_conn<'a>(transaction: &mut DbConnection<'a>, user: InsertableUser<'a>) -> Result<User, DataError>{
        let user = diesel::insert_into(users::table)
            .values(user)
            .get_result::<User>(transaction)
            .await?;
    ...
    pub async fn get_accept_conn<'a>(transaction: &mut DbConnection<'a>, name: &'a str) -> Result<User, DataError>{
        let user = users::table.filter(users::name.eq(name))
            .first::<User>(transaction)
            .await?;
        Ok(user)
