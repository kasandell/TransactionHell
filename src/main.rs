...
async fn main() {
    ...
    let name_a = "Kyle";
    let name_b = "Not Kyle";
    let user_a = db::connection().await.expect("connection").transaction::<_, DataError, _>(|txn| async move {
        let user = User::insert_accept_conn(txn, InsertableUser {
            name: name_a
        }).await?;
        Ok(user)
    ...
    let failure: Result<User, DataError> = db::connection().await.expect("connection").transaction(|txn| async move {
        let user = User::insert_accept_conn(txn, InsertableUser {
            name: name_b
        }).await?;
        let found = User::get_accept_conn(txn, name_b).await?;
